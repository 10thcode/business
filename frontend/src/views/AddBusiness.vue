<script setup>
  ...
  async function addBusiness () {
    ...
    sendNotification(user.value.name, data.name);
    loading.value = false;
    router.push(`/businesses/${data.id}`);
  }


  async function sendNotification(username, business) {
    const title = "Business Created Successfully!";
    const content = `
...
This is Business!`
    ...
    const response = fetch(`${API_URL}/api/v1/users/${user.value.sub}/notifications/`, {
      method: 'POST',
      body: formData
    });
  }
  ...
  <section class="form-container">
    <form @submit.prevent="addBusiness">
      ...
      <div class="form-buttons action-buttons">
        <IconButton @click="$router.push('/')" color="accent" icon="close"/>
        <IconButton
          color="solid-accent"
          type="submit"
          icon="add">Add &nbsp</IconButton>
      </div>
    </form>
  </section>
</template>
