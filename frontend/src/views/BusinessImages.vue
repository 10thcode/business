<script setup>
    ...
    if (files) {
      for (const img of files) {
        new Compressor(img, {
          ...
          error: (err) => {
            throw new Error(`Image Compression Error: ${err}`);
          }
        });
      }
    }
  ...
  async function deleteImage(imageId) {
    loading.value = true;
    const response = await fetch(`${API_URL}/api/v1/businesses/${businessId}/images/${imageId}/`, {
      method: 'DELETE',
    });
...
<template>
  ...
  <PageTitle>Business Images</PageTitle>
