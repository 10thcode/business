...
@api_view(["GET", "PUT", "DELETE"])
def notification_details(request, notification_id, format=None):
    """
    Handle HTTP requests for /notifications/<notification_id> endpoint.
    ...
    if request.method == "GET":
        serializer = NotificationSerializer(product_review)
        return Response(serializer.data)
...
def user_notification_list(request, format=None):
    """
    Handle HTTP requests for /users/notifications
...
@api_view(["GET", "PUT", "DELETE"])
def user_notification_details(request, user_notification_id, format=None):
    """
    Handle HTTP requests for
